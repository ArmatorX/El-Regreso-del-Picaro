...
namespace Tests
{
    public class EntidadUnitTests
    {
        ...
        [Test]
        public void Entidad_calcularDaño_DevuelveValorCorrectoMelé()
        {
            int modificador = 2;

            int dañoEsperado = 2 + modificador;
            int dañoReal = entidad.calcularDaño(false, false);

            Assert.AreEqual(dañoEsperado, dañoReal);
        }

        [Test]
        public void Entidad_calcularDaño_DevuelveValorCorrectoDistancia()
        {
            int dañoEsperado = 2;
            int dañoReal = entidad.calcularDaño(false, true);

            Assert.AreEqual(dañoEsperado, dañoReal);
        }

        [Test]
        public void Entidad_calcularDaño_MultiplicaPorDosSiCrítico()
        {
            int modificador = 2;

            int dañoEsperado = 2 * (2 + modificador);
            int dañoReal = entidad.calcularDaño(true, false);
